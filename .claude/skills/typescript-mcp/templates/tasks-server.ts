...
server.registerTool(
	...
	{
		description:
			"Analyzes a large dataset. Returns immediately with taskId for long-running operations.",
...
server.registerTool(
	...
	{
		description:
			"Processes items in batch, may pause for confirmation on certain items",
		inputSchema: z.object({
			items: z.array(z.string()).describe("Items to process"),
			requireConfirmation: z
				.boolean()
				.default(false)
				.describe("Pause for confirmation on flagged items"),
		}),
	...
	async ({ items, requireConfirmation }, extra) => {
		if (!extra?.task) {
			return {
				content: [
					{ type: "text", text: "Error: This tool requires task mode" },
				],
				isError: true,
			};
		}
...
async function performAnalysis(
	...
	const steps =
		analysisType === "full" ? 5 : analysisType === "detailed" ? 3 : 1;
